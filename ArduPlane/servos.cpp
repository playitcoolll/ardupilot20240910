                                   ...
                                   SRV_Channel::Aux_servo_function_t func1_out, SRV_Channel::Aux_servo_function_t func2_out) const
{
    ...
    float out1 = constrain_float((in2 - in1) , -2500, 2500);
    float out2 = constrain_float((in2 + in1) , -2500, 2500);
...
void Plane::dspoiler_update(void)
{
    ...
    if (rudder >= 0) {
        ...
        dspoiler_outer_right = constrain_float(dspoiler_outer_right + rudder, 0, 2500);
        dspoiler_inner_right = constrain_float(dspoiler_inner_right - 0.5 * rudder, -2500, 2500);
        dspoiler_outer_left = 0;
        // dspoiler_inner_left = 0;
    } else {
        // apply rudder to left wing
        dspoiler_outer_right = 0;
        // dspoiler_inner_right = 0;
        dspoiler_outer_left = constrain_float(dspoiler_outer_left - rudder, 0, 2500);
        dspoiler_inner_left = constrain_float(dspoiler_inner_left + 0.5 * rudder, -2500, 2500);
