...
float AP_PitchController::_get_rate_out(float desired_rate, float scaler, bool disable_integrator, float aspeed,bool ground_mode)
{
    ...
    bool underspeed = aspeed <= float(8);//飞翼
    ...
    // hal.console -> printf("%fn",myaoa._aoa);
    ...
    if(_ahrs.get_velocity_NED(Ve)){
        ...
        boost_az =0.1-0.1/40.0*aspeed;

        boost_az = constrain_float(boost_az , 0.001,0.1);
    ...
    boost_aoa =0.8f-0.2f/40.0f*aspeed;

    boost_aoa = constrain_float(boost_aoa , 0.6f,0.8f);
    ...
    if (aoa <= 7.0f)
    {
        ...
        error_aoa =0 - pow(aoa,1.5f);
    }



    out_aoa = boost_aoa * error_aoa*0 + boost_aoa * my_aoa;
    ...
    float ki_pitch = 0.3;
    // if(aspeed > 0.0f){
    //     ki_pitch =0.3 - 0.3f/40*((abs(aspeed)));
    // }
    ...
    float out_my = out_my_p + I_term + D_term  + out_aoa * scaler + out_az * scaler;//
    ...
    // float out_my = out_my_p  + I_term  + D_term  +  out_aoa /57.3 +  out_az /57.3;//乘k也可以 /57.3也行
    // previous_error = error_my;
...
/*
  AC_PID based rate controller
*/
float AP_PitchController::_get_rate_out_my(float desired_rate, float scaler, bool disable_integrator, float aspeed,bool ground_mode,float k1, float k3)
{
    ...
    bool underspeed = aspeed <= float(3);//飞翼
    ...
    //     aoa = 0;
    // }
    aoa = constrain_float(aoa , -30.0F,30.0F);


    const aoassa &myaoa = *AP::Aoassa();
    float my_aoa = 0 - myaoa._aoa;
    my_aoa = constrain_float(my_aoa , -30.0F,30.0F);
    ...
    //     boost_az =0.01-0.01/40.0*aspeed;

    //     boost_az = constrain_float(boost_az , 0.001,0.01);

    
    //     // hal.console ->printf("%f\n",a_relative_e2hangji.z);
    //     out_az = boost_az * (a_relative_e2hangji.z) * 0;
    // }
    ...
    boost_aoa =0.8f-0.2f/60.0f*aspeed;
    ...
    float boost_pitch_p = 0.2f;
    if(aspeed > 0.0f){
        boost_pitch_p =0.2f - 0.2f/60.0f*((abs(aspeed)));
    }
    boost_pitch_p = constrain_float(boost_pitch_p, 0.005f,0.2f);
    //计算P
    error_my = desired_rate-degrees(rate_y);
    float out_my_p = boost_pitch_p * error_my*k3;//

    //计算I
    float ki_pitch = 0.0;//0.3
    ...
    integral +=  ki_pitch * error_my * dt*k3*0;//
    integral= constrain_float(integral, -15.0F,15.0F);  //如果拉杆起不来就是这个地方小了
    float I_term =  integral;
    ...
    // derivative = (error_my - previous_error) / dt;
    // D_term = boost_pitch_d * derivative;//*k3



    //飞翼
    float out_my = out_my_p + I_term*0  + out_aoa  *k1;//*k3 
    ...
    pinfo.target = -my_aoa + aoa*0;
    pinfo.actual = -my_aoa + aoa*0;
    ...
    //手动模式加增稳用
float AP_PitchController::_get_rate_out_my(float desired_rate, float scaler, bool disable_integrator, float aspeed,bool ground_mode,float k1, float k3, int k4, float elevator)
{
    ...
    float out_my_p = boost_pitch_p * error_my*k3;//*k3


    //计算I
    float ki_pitch = 0.0;//0.3

    if(abs(int(elevator)) <= 200){
            ki_pitch = 0.0f;
    }else{
            ki_pitch = 0.0f;
            integral = 0.0f;
    ...
    // hal.console -> printf("%f,%f\n",integral,ki_pitch);
    integral +=  ki_pitch * error_my * dt*k3;//*k3
    integral= constrain_float(integral, -15.0F,15.0F);  //如果拉杆起不来就是这个地方小了
    float I_term =  integral;
    ...
    float out_my;
    if(underspeed){
        out_my = out_my_p + out_aoa *k1;
    }else{
        out_my = out_my_p + I_term *0 + out_aoa *k1;
    }
    ...
    float k_out = 1;
    out_my = k_out * out_my;
    out_my = constrain_float(out_my, -40,40);
    ...
    // hal.console -> printf("%f\n",elevator);
    return constrain_float(elevator + out_my * 100, -4500, 4500);
}
...
float AP_PitchController::get_servo_out(int32_t angle_err, float scaler, bool disable_integrator, bool ground_mode)
{
    ...
    float k_sp = 0.1f;
    if(aspeed > 0.0f){
        k_sp =5.0f - 5.0f/40.0f*((abs(aspeed)));//
    }
    k_sp = constrain_float(k_sp, 0.01f,5.0f);

    // hal.console -> printf("%f\n",k_sp);

    angle_err_deg = angle_err * 0.01;
    float desired_rate = angle_err_deg * k_sp;//
...
float AP_PitchController::get_servo_out(int32_t angle_err, float scaler, bool disable_integrator, bool ground_mode,float k1,float k2,float k3)
{
    // Calculate offset to pitch rate demand required to maintain pitch angle whilst banking
    // Calculate ideal turn rate from bank angle and airspeed assuming a level coordinated turn
    // Pitch rate offset is the component of turn rate about the pitch axis
    ...
    float k_sp = 5.0f;
    if(aspeed > 0.0f){
        k_sp =5.0f - 5.0f/60.0f*((abs(aspeed)));//
    }
    k_sp = constrain_float(k_sp, 0.01f,5.0f);
    ...
    angle_err_deg = angle_err * 0.01;
    float desired_rate = angle_err_deg * k_sp * k2;//

    // hal.console -> printf("%f\n",k2);


    return _get_rate_out_my(desired_rate, scaler, disable_integrator,  aspeed, ground_mode,  k1,  k3);
}
...
// float AP_PitchController::get_servo_out(int32_t angle_err, float scaler, bool disable_integrator, bool ground_mode,float k1,float k2,float k3, float k4)
// {
//     // Calculate offset to pitch rate demand required to maintain pitch angle whilst banking
//     // Calculate ideal turn rate from bank angle and airspeed assuming a level coordinated turn
//     // Pitch rate offset is the component of turn rate about the pitch axis
...
//     return _get_rate_out_my(desired_rate, scaler, disable_integrator,  aspeed, ground_mode,  k1,  k3,k4);
// }


float AP_PitchController::get_servo_out(int32_t angle_err, float scaler, bool disable_integrator, bool ground_mode,float k1,float k2,float k3, int k4, float elevator)
{
    // Calculate offset to pitch rate demand required to maintain pitch angle whilst banking
    // Calculate ideal turn rate from bank angle and airspeed assuming a level coordinated turn
    // Pitch rate offset is the component of turn rate about the pitch axis
    ...
    return _get_rate_out_my(desired_rate, scaler, disable_integrator,  aspeed, ground_mode,  k1,  k3,k4,elevator);
}




void AP_PitchController::reset_I_my()
{
    integral = 0.0f;
}
