...
void AP_AHRS::update_AOA_SSA(void)
{
    ...
    if (now - _last_AOA_update_ms < 10) {
        // don't update at more than 100Hz
        return;
    }
    ...
    //sitl中会自己给你带风场
    aoa_velocity = aoa_velocity - aoa_wind;//- aoa_wind
    ...
    //自己的
    const aoassa &my_log = *AP::Aoassa();
    float my_aoa_log = my_log._aoa;
    float my_ssa_log = my_log._ssa;
    ...
    // 自己的
    _AOA = _AOA  + my_aoa_log * 0;
    _SSA = _SSA  + my_ssa_log * 0;
#endif
}
